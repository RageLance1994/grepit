...
router.get('/', (req, res) => {
  ...
  const reminders = [
    { label: 'Verifica registrazioni bancarie', deadline: 'Settimanale' },
    { label: 'Aggiorna lista clienti', deadline: 'Ogni 2 settimane' },
    { label: 'Invia solleciti', deadline: 'Ogni venerdì' },
  ];

  const quickLinks = [
    { label: 'Crea fattura', href: '/contabilita/fatture' },
    { label: 'Registra movimento', href: '/contabilita/prima-nota' },
    { label: 'Previsioni cassa', href: '/contabilita/cashflow' },
  ];
...
router.get('/cashflow', (req, res) => {
  ...
  const rules = [
    { label: 'Abbassa il burn rate se il saldo scende sotto € 100k' },
    { label: 'Evidenzia incassi oltre € 30k', color: 'success' },
    { label: 'Avvisa se previsioni negative per due mesi consecutivi', color: 'error' },
  ];
